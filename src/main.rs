...
// fn device_get(instance: &Arc<Instance>, surface: &Arc<Surface>) -> (Arc<Device>, Arc<Queue>) {
...
// let (device, queue) = device_get(&instance, &surface);
// let (mut swapchain, images) = swapchain_get(&device, &surface, &window);
//
